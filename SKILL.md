...
- **聚焦模式**：用户首条消息即明确指定深挖/复现/缺陷扫描目标，且不存在 `_context_checkpoint.json` 时，先执行 S1 最低限度全景（仅技术栈 + 入口点，跳过异常信号扫描与完整模块映射），随后直接跳转 S5/S6/S6B；跳转前向用户确认一次"将跳过全量全景，仅做聚焦分析"。
