...
| 评估工程风险、扫描源码缺陷、给出改进建议 | 将建议直接落地为代码改动 |
| 生成架构分析报告、功能复现指南、迁移方案 | 代替用户做设计决策——所有关键节点必须经过确认 |
...
不要被 8 阶段流水线吓到。你只需要回答一个问题：**"你现在想做什么？"**

### 典型场景速查表

| 你的需求 | 直接说 | 系统会跑什么 | 耗时（token） |
|----------|--------|-------------|--------------|
| 🧭 "我想了解这个项目整体架构" | `分析 XX 项目` | 全量模式 S1→S4 → 人工确认 → 报告 | 高 |
| 🔍 "这个算法/机制是怎么实现的" | `深挖 XX 算法` | 聚焦模式：S1 快速全景 → 直接 S5 深挖 | 中 |
| 📋 "这个功能怎么复用/复刻" | `复现 XX 功能` | 聚焦模式：S1 快速全景 → 直接 S6 复现 | 中 |
| 🐛 "帮我检查一下代码质量/安全隐患" | `扫描 XX 项目缺陷` | 全量模式 S1→S4 → 勾选 S6B 缺陷扫描 | 高 |
| 🚀 "我只要快速报告，不要深挖" | `分析 XX 项目，跳过深挖` | 全量模式 S1→S4 → 跳过 S5/S6 → S7 报告 | 中 |
